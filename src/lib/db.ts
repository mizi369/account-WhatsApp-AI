...
export const db = {
  // Initialize: Fetch all data from Firestore and cache in LocalStorage
  init: async () => {
    ...
    try {
        ...
        try {
            ...
            console.warn('[DB] Firestore connection test failed (likely offline/new project)');
        }
        ...
        await Promise.all(tableKeys.map(async (table) => {
            try {
                ...
                // If table doesn't exist or permissions fail, fallback to empty
    ...
    try {
      ...
      items = items.map((i: any) => {
        if (i.id === undefined || i.id === null) {
            i.id = Date.now().toString() + Math.floor(Math.random() * 1000);
            changed = true;
        }
        return i;
      });
  ...
  insert: async <T>(table: string, item: T) => {
    ...
    const id = newItem.id ? String(newItem.id) : Date.now().toString();
    newItem.id = id;
    ...
    try {
        ...
        console.error(`[DB] Insert Failed ${table}:`, e);
        try {
  ...
  update: async <T extends { id: string | number }>(table: string, id: string | number, updates: Partial<T>) => {
    ...
    try {
        await updateDoc(doc(firestore, table, String(id)), updates as any);
        console.log(`[DB] Updated ${table} : ${id}`);
        return { error: null };
    } catch (e: any) {
        console.error(`[DB] Update Failed ${table}:`, e);
        try {
  ...
  delete: async <T extends { id: string | number }>(table: string, id: string | number) => {
    ...
    try {
        await deleteDoc(doc(firestore, table, String(id)));
  ...
  getStats: () => {
    ...
    const totalInventoryValue = inventory.reduce((sum, i) => sum + (i.buyPrice * (i.stock || 0)), 0);
