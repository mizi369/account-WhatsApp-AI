...
import { Lock, Mail, ChevronRight, UserPlus, ShieldCheck, HelpCircle, RefreshCw, LogIn } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      if (email && password && name) {
        const usersRaw = localStorage.getItem('mnf_users');
        const users = (usersRaw && usersRaw !== 'undefined') ? JSON.parse(usersRaw) : [];
        if (!Array.isArray(users)) {
            // Fix corrupted state
            localStorage.setItem('mnf_users', '[]');
      ...
      if (!Array.isArray(users)) {
          localStorage.setItem('mnf_users', '[]');
          setError('Sistem data pengguna tamat tempoh. Sila cuba lagi.');
          return;
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user) {
        localStorage.setItem('mnf_auth_type', 'firebase');
        localStorage.setItem('mnf_role', 'admin'); // Default role for Google users
        localStorage.setItem('mnf_admin_name', result.user.displayName || 'User');
        if (result.user.photoURL) localStorage.setItem('mnf_admin_image', result.user.photoURL);
        onLogin();
      }
    } catch (err: any) {
            ...
            <>
              ...
              {!isSignUp && (
                <div className="relative py-4">
                  ...
                  <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest">
                    <span className="bg-white px-4 text-slate-300">Atau</span>
                  </div>
                </div>
              )}
              ...
              {!isSignUp && (
                <button 
                  ...
                  className="w-full bg-white border-2 border-slate-100 text-slate-600 py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-sm hover:bg-slate-50 hover:border-slate-200 transform hover:-translate-y-1 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
                >
                  {isGoogleLoading ? (
