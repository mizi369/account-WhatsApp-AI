...
const WhatsAppMonitor: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-darker -m-4 md:-m-6 lg:-m-8 overflow-hidden font-sans">
      ...
      <div className="bg-slate-900 text-white px-3 py-2 flex justify-between items-center shadow-md z-20 h-14 shrink-0 border-b border-white/5">
         <div className="flex items-center gap-2 md:gap-3">
            <div className={`w-8 h-8 md:w-9 md:h-9 rounded-full bg-slate-800 overflow-hidden border border-white/10 shadow-sm shrink-0 ${activeTab !== 'dashboard' ? 'flex md:hidden' : 'flex'}`}>
               {adminInfo.image ? <img src={adminInfo.image} className="w-full h-full object-cover" alt="Admin" /> : <div className="w-full h-full flex items-center justify-center text-slate-500"><User size={16}/></div>}
            </div>
            <div className="flex flex-col md:flex-row md:items-center md:gap-2 leading-tight">
                <h1 className="font-bold text-[10px] md:text-sm tracking-wide uppercase truncate max-w-[80px] md:max-w-none">Wa Hub</h1>
                <span className="opacity-70 text-[8px] hidden md:inline">v2.4 Ent</span>
            </div>
         </div>
         ...
         <div className="flex items-center gap-1.5 md:gap-4">
            ...
            <div className="relative">
                <button 
                    ...
                    className={`p-1.5 md:p-2 rounded-full transition-all relative ${complaints.length > 0 ? 'bg-red-500/10 text-red-400 animate-pulse' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                    title="Aduan Pelanggan"
                >
                    <Bell size={16} className="md:size-[18px]" />
                    {complaints.length > 0 && (
                        <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[7px] font-black w-3.5 h-3.5 flex items-center justify-center rounded-full border border-slate-900">
                            {complaints.length}
                        </span>
                    )}
                </button>
      ...
      {activeTab === 'connection' && (
          <div className="flex-1 overflow-y-auto lg:overflow-hidden bg-slate-100 flex items-start md:items-center justify-center p-2 sm:p-4 lg:p-8">
              <div className="bg-white w-full max-w-5xl md:h-[580px] rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row relative">
                  
                  {/* LEFT SIDE: QR / STATUS VISUAL */}
                  <div className="md:w-1/2 bg-slate-900 p-3 md:p-8 flex flex-col justify-center items-center relative overflow-hidden text-white min-h-[160px] md:min-h-0">
                      ...
                      <div className="relative z-10 flex flex-col items-center">
                          {isConnected ? (
                              <div className="w-20 h-20 md:w-48 md:h-48 bg-emerald-500/20 rounded-xl md:rounded-[2rem] flex items-center justify-center border-4 border-emerald-500 animate-pulse">
                                  <CheckCircle size={32} className="md:size-80 text-emerald-400 drop-shadow-lg" />
                              </div>
                          ) : isScan && qrCode ? (
                              <div className="bg-white p-1 md:p-4 rounded-lg md:rounded-3xl shadow-2xl border-2 md:border-4 border-cyan-500/30">
                                  {/* Reduced QR code size for easier mobile scanning as requested */}
                                  <div className="bg-white p-0.5 md:p-2 rounded-lg">
                                    <img src={qrCode} className="w-20 h-20 xs:w-24 xs:h-24 sm:w-40 sm:h-40 md:w-56 md:h-56 object-contain bg-white rounded-md" alt="WhatsApp QR Code" />
                                  </div>
                              </div>
                          ) : (
                              <div className="w-20 h-20 md:w-48 md:h-48 bg-slate-800 rounded-xl md:rounded-[2rem] flex items-center justify-center border-4 border-slate-700">
                                  {isReconnecting ? <Loader2 size={24} className="md:size-60 text-slate-600 animate-spin" /> : <ScanLine size={24} className="md:size-60 text-slate-600" />}
                              </div>
                          )}
                          
                          <div className={`mt-2 md:mt-8 px-3 md:px-6 py-0.5 md:py-2 rounded-full font-black text-[7px] md:text-xs uppercase tracking-[0.2em] flex items-center gap-2 ${isConnected ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>
                              <Activity size={8} className={isConnected ? "animate-bounce" : ""} />
                              {status}
                          </div>
                      </div>
                  </div>
                  ...
                  {/* RIGHT SIDE: CONTROLS */}
                  <div className="md:w-1/2 p-4 md:p-10 flex flex-col justify-between space-y-4 md:space-y-0">
                      <div>
                          <h2 className="text-sm md:text-xl font-black text-slate-900 uppercase tracking-tighter leading-none mb-1">Connect WhatsApp<br/><span className="text-cyan-600">Super Admin</span></h2>
                          <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-normal">
                      ...
                      <div className="space-y-1.5 md:space-y-4">
                          ...
                          {(isOfflineOrChecking || isScan) && (
                              <button onClick={handleConnect} className="w-full bg-slate-900 text-white py-2 md:py-4 rounded-xl font-bold uppercase tracking-widest text-[7px] md:text-[10px] hover:bg-cyan-600 transition-all shadow flex items-center justify-center gap-1.5 md:gap-3 active:scale-95">
                                  <Power size={10} className="md:size-16" /> {isScan ? "Refresh QR" : "Connect Super Admin"}
                              </button>
                          )}
                          
                          {(isConnected || isReconnecting || isScan) && (
                              <button onClick={handleDisconnect} className="w-full bg-red-50 text-red-500 py-2 md:py-4 rounded-xl font-bold uppercase tracking-widest text-[7px] md:text-[10px] hover:bg-red-100 transition-all border border-red-100 flex items-center justify-center gap-1.5 md:gap-3 active:scale-95">
                                  <Power size={10} className="md:size-16" /> {isScan ? "Cancel Setup" : "Disconnect System"}
                              </button>
                          )}

                          {/* REFRESH QR / FORCE SYNC SECTION */}
                          <div className="flex gap-2">
                              <button onClick={handleRefreshQR} className="flex-1 bg-cyan-50 text-cyan-600 py-2 md:py-2 rounded-lg md:rounded-xl font-bold uppercase tracking-widest text-[7px] md:text-[8px] hover:bg-cyan-100 transition-all flex items-center justify-center gap-2 border border-cyan-100 shadow-sm active:scale-95">
                                  <RefreshCw size={10} className="animate-spin-slow" /> Refresh QR
                              </button>
                               <button onClick={() => { socket.emit('cmd-refresh-profile'); alert('Permintaan sinkronasi profil dihantar...'); }} className="flex-1 bg-emerald-50 text-emerald-600 py-2 md:py-2 rounded-lg md:rounded-xl font-bold uppercase tracking-widest text-[7px] md:text-[8px] hover:bg-emerald-100 transition-all flex items-center justify-center gap-2 border border-emerald-100 shadow-sm active:scale-95">
                                  <User size={10} /> Sync Profile
                              </button>
                          </div>
                      </div>

                      {/* MINI TERMINAL */}
                      <div className="bg-slate-50 border border-slate-200 rounded-xl md:rounded-2xl p-3 md:p-4 flex flex-col h-20 md:h-32 relative">
                          <div className="flex items-center gap-2 mb-1 md:mb-2 pb-1 md:pb-2 border-b border-slate-200">
