...
import { ErrorBoundary } from './SafeContainer';
...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 max-w-7xl mx-auto pb-8 p-1 md:p-2">
      
      {/* DASHBOARD UNTUNG */}
      <div className="mt-1 md:mt-2 mb-4 md:mb-8">
        <ErrorBoundary>
          <ProfitDashboard />
        </ErrorBoundary>
      </div>
      ...
      <p className="text-[9px] md:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 mt-4">Status Operasi</p>
...
const MiniStat: React.FC<{icon: any, label: string, value: string, color: 'cyan'|'indigo'|'emerald'|'orange'}> = ({icon, label, value, color}) => {
    ...
    return (
        <div className={`p-2.5 md:p-3 rounded-xl border ${colors[color]} flex flex-col justify-between h-20 md:h-24 relative overflow-hidden group hover:shadow-[0_0_15px_rgba(0,188,212,0.6)] transition-all glass-panel cursor-default`}>
            <div className="absolute right-1 top-1 opacity-10 scale-125 md:scale-150 group-hover:rotate-12 transition-transform text-white">{icon}</div>
            <div className="flex items-center gap-1.5 mb-0.5 md:mb-1 relative z-10">
                <div className="p-1 md:p-1.5 bg-white/10 rounded-lg shadow-sm backdrop-blur-md border border-white/5 [&>svg]:w-3 md:[&>svg]:w-4 [&>svg]:h-3 md:[&>svg]:h-4">{icon}</div>
                <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest opacity-80 truncate">{label}</span>
            </div>
            <div className="relative z-10">
                <p className="text-sm md:text-lg font-black tracking-tight leading-none text-white">{value}</p>
            </div>
        </div>
    );
};
