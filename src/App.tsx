...
import { ErrorBoundary, LoadingScreen, ServerOfflineAlert } from './components/SafeContainer';
...
const App: React.FC = () => {
  ...
  const syncLiveSlotsToAi = async () => {
      try {
          const getAvailabilityForDate = async (dateStr: string) => {
              ...
              const teams = (teamsRaw && teamsRaw !== 'undefined') ? JSON.parse(teamsRaw) : [];
              const activeTeams = Array.isArray(teams) ? teams.filter((t: any) => t.active) : [];
              
              const slotsRaw = localStorage.getItem('mnf_time_slots');
              const slots = (slotsRaw && slotsRaw !== 'undefined') ? JSON.parse(slotsRaw) : [];
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
        if (user) {
            setIsAuthenticated(true);
            localStorage.setItem('mnf_auth', 'true');
            // If it's a first time google login, these might not be set yet in the current session
            if (!localStorage.getItem('mnf_admin_name')) {
                localStorage.setItem('mnf_admin_name', user.displayName || 'User');
            }
        } else {
            // Only force logout if we specifically want strict firebase auth
            // For now, let's allow the local storage fallback to persist for better UX in dev
            // but update the status if they actually log out.
            if (localStorage.getItem('mnf_auth_type') === 'firebase') {
                setIsAuthenticated(false);
                localStorage.removeItem('mnf_auth');
            }
        }
    ...
    const initSystem = async () => {
        ...
        // FAIL-SAFE: Force loading screen to end after 10 seconds regardless of sync status
        const forceLoadTimeout = setTimeout(() => {
            if (isLoading) {
                console.warn('[SYSTEM] Loading taking too long, forcing start...');
                setIsLoading(false);
            }
        }, 10000);

        try {
          const success = await db.init(); // Fetch data from Supabase
          if (!success) setIsServerOffline(true);
        } catch (e) {
        ...
        // Fetch Admin Info from Backend with timeout safety
        ...
        fetch('/api/admin', { signal: controller.signal })
          ...
          .catch(err => {
            clearTimeout(timeoutId);
            console.warn('Backend server seems offline, using local data.', err);
            setIsServerOffline(true);
          });
    };
    initSystem();
    ...
    const handleAutoBooking = async (data: any) => {
        ...
        const teamsRaw = localStorage.getItem('mnf_teams');
        if (teamsRaw && teamsRaw !== 'undefined') {
        ...
        try {
            ...
            const currentBookings: Booking[] = (bookingsRaw && bookingsRaw !== 'undefined') ? JSON.parse(bookingsRaw) : [];
            
            if (!Array.isArray(currentBookings)) {
                localStorage.setItem(TABLES.BOOKINGS, '[]');
                throw new Error("Invalid bookings data; reset to empty array");
            }
  ...
  // Loading Screen using safe component
  if (isLoading) {
  ...
  return (
    <ErrorBoundary>
      {isServerOffline && <ServerOfflineAlert />}
      <HashRouter
      future={{
    ...
    >
      <div className="flex h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
        ...
        <div className={`transition-all duration-300 ease-in-out border-r border-slate-800 bg-slate-900 flex flex-col z-50 ${isSidebarOpen ? 'w-72 md:w-64' : 'w-0 md:w-20'} fixed md:relative h-full shadow-2xl overflow-hidden`}>
          
          {/* Logo Section */}
          <div className={`h-24 md:h-20 flex items-center justify-between border-b border-slate-800 relative ${isSidebarOpen ? 'px-6' : 'px-2'}`}>
             ...
             <button 
                ...
                className={`absolute -right-3 top-10 md:top-7 bg-slate-800 text-slate-400 p-2 md:p-1.5 rounded-full border border-slate-700 shadow-md hover:text-white transition-all z-50 flex items-center justify-center min-w-[40px] min-h-[40px] md:min-w-0 md:min-h-0 ${!isSidebarOpen && 'hidden md:flex'}`}
             >
        ...
        {/* --- MAIN CONTENT AREA --- */}
        <div className="flex-1 bg-slate-950 relative overflow-hidden flex flex-col h-full w-full">
           
           {/* GLOBAL WHATSAPP AI STATUS HEADER */}
           <div className="bg-slate-900 border-b border-slate-800 px-4 py-3 md:py-2 flex justify-between items-center z-40 sticky top-0 shadow-md h-16 md:h-12 shrink-0">
               <div className="flex items-center gap-3">
                   {/* Mobile Menu Trigger */}
                   <button 
                      onClick={() => setIsSidebarOpen(true)}
                      className={`md:hidden p-2 text-slate-400 hover:text-white bg-slate-800 rounded-lg ${isSidebarOpen ? 'hidden' : 'flex'}`}
                   >
                       <Menu size={20} />
                   </button>
                   <div className="flex items-center gap-2">
               ...
               <div className="flex items-center gap-2 md:gap-3">
                   {!isSocketConnected && (
                       <div className="flex items-center gap-2 px-2 md:px-3 py-1 bg-red-500/10 border border-red-500/20 rounded-lg animate-pulse">
                           <WifiOff size={10} className="md:size-12 text-red-500"/>
                           <span className="text-[8px] md:text-[9px] font-black text-red-500 uppercase tracking-widest hidden xs:inline">Offline</span>
                       </div>
                   )}
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                       <Clock size={12} className="hidden xs:block"/> {sysTime}
                   </p>
               </div>
           </div>
